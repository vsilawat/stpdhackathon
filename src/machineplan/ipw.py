...
def op_diameter(op: Op) -> float:
    d, pilot = op.feature.diameter, op.tool_diameter is None
    match op.name:
        case "GUN_DRILL_THROUGH_HOLE": return GUN_RATIO * d
        case "SPADE_DRILL_TO_ENLARGE_THROUGH_HOLE": return SPADE_RATIO * d
        case "DRILL_THROUGH_HOLE_INTO_CENTER" if pilot: return PILOT_RATIO * d
        case "DRILL_BLIND_HOLE_INTO_CENTER" if pilot: return GUN_RATIO * d
        case _: return op.tool_diameter or d
...
def op_solid(part: Part, op: Op, found: Features) -> m3d.Manifold | None:
    ...
    h = op.feature
    if n == "SPOT_DRILL": return solids.spot_solid(h.x, h.y, 0.0, top, min(0.3 * h.diameter + 0.5, 6.0))
    r = op_diameter(op) / 2
